...
def parse_args():
    ...
    # loss
    parser.add_argument('--loss', default='CombinedLoss',
                        choices=LOSS_NAMES,
                        help='loss: ' +
                        ' | '.join(LOSS_NAMES) +
                        ' (default: BCEDiceLoss)')
    ...
    with torch.no_grad():
        ...
        for input, target, _ in val_loader:
            ...
            if config['deep_supervision']:
                ...
                prec = precision(outputs, target)
                rec = recall(outputs, target)
                f1 = f1_score(outputs, target)
...
def main():
    ...
    for epoch in range(config['epochs']):
        ...
        print('loss %.4f - iou %.4f - val_loss %.4f - val_iou %.4f - dice %.4f - val_dice %.4f - acc %.4f - val_acc %.4f - prec %.4f - val_prec %.4f - rec %.4f - val_rec %.4f - f1 %.4f - val_f1 %.4f'
              % (train_log['loss'], train_log['iou'], val_log['loss'], val_log['iou'], train_log['dice'], val_log['dice'], train_log['acc'], val_log['acc'], train_log['prec'], val_log['prec'], train_log['rec'], val_log['rec'], train_log['f1'], val_log['f1']))
        ...
        log['prec'].append(train_log['prec'])
        log['rec'].append(train_log['rec'])
        ...
        log = {key: [value.item() if isinstance(value, torch.Tensor) else value for value in log[key]] for key in log}

        with open('models/%s/log.csv' % config['name'], 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(log.keys())
            writer.writerows(zip(*log.values()))
        trigger += 1

        if (val_log['iou'] > best_iou) or (val_log['dice'] > best_dice) or (val_log['acc'] > best_acc) or (val_log['prec'] > best_prec) or (val_log['rec'] > best_rec) or (val_log['f1'] > best_f1):
            torch.save(model.state_dict(), 'models/%s/model.pth' %
                       config['name'])
            best_iou = val_log['iou']
            best_dice = val_log['dice']
            best_acc = val_log['acc']
            best_prec = val_log['prec']
            best_rec = val_log['rec']
            best_f1 = val_log['f1']
